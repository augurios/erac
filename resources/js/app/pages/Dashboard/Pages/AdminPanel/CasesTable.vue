    ...
    <div class="md-layout-item">
      <md-card>
        <md-card-header class="md-card-header-icon md-card-header-green">
          ...
          <h4 class="title">Casos en Archivo</h4>
        </md-card-header>
        <md-card-content>
          <md-table
            :value="queriedData"
          ...
          >
            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              ...
              <md-table-cell md-label="Mediador(a)" md-sort-by="mediator">{{ item.mediator }}</md-table-cell>
              <md-table-cell md-label="Solicitante" md-sort-by="opener">{{ item.opener }}</md-table-cell>
              <md-table-cell md-label="Solicitado" md-sort-by="recipient">{{ item.recipient }}</md-table-cell>
...
export default {
  ...
  data() {
    return {
      ...
      footerTable: ["Estado", "Id", "Monto", "Fecha", "Mediador(a)", "Solicitante", "Solicitado", "Acciones"],
      searchQuery: "",
  ...
  methods: {
    ...
    handleDelete(item) {
      Swal.fire({
        ...
        axios.delete(`case/${item.id}`).then((response) => {
          if (result.value) {
            this.deleteRow(item);
            Swal.fire({
              title: "Deleted!",
              text: `You deleted Case ID ${item.id}`,
              type: "success",
              confirmButtonClass: "md-button md-success btn-fill",
              buttonsStyling: false
            });
